...
void ExtInt_Mcu::setEdge( Edge edge)
{
  switch( edge )
  {
    case FALLING:  EXTI->FTSR1 |=  (1 << pinId);
                   EXTI->RTSR1 &= ~(1 << pinId);
                   break;

    case RISING:   EXTI->FTSR1 &= ~(1 << pinId);
                   EXTI->RTSR1 |=  (1 << pinId);
                   break;

    case BOTH:     EXTI->FTSR1 |= (1 << pinId);
                   EXTI->RTSR1 |= (1 << pinId);
                   break;

    default:       EXTI->FTSR1 &= ~(1 << pinId);
                   EXTI->RTSR1 &= ~(1 << pinId);
                   break;
  }
}
