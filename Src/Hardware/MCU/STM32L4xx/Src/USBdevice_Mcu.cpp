...
//*******************************************************************
#define USB_BUF_BASE         (USB_PMAADDR)
...
{
  SystemInit_48MHzClock();
    
  PinConfig::set( PinConfig::USB_DP, PinConfig::NO_PUPD
                                    |PinConfig::HIGH_SPEED
                                    |PinConfig::PUSH_PULL );

  PinConfig::set( PinConfig::USB_DM, PinConfig::NO_PUPD
                                    |PinConfig::HIGH_SPEED
                                    |PinConfig::PUSH_PULL );
  usbPtr = this;
}
...
inline void USBdevice_Mcu::isr(void)
{
  ...
  if( istr & USB_ISTR_CTR )
  {
    ...
    {
      if( USB->EP[num]&USB_EP_CTR_RX )
      {
        EP[num].clrCTR_RX();
        eventHandler( num|0x00,                                   // calc EP address
                      (USB->EP[num]&USB_EP_SETUP)?true:false ); // Setup?
     }

      if( USB->EP[num]&USB_EP_CTR_TX )
      {
        EP[num].clrCTR_TX();
...
void USBdevice_Mcu::start(void)
{
  ...
  NVIC_EnableIRQ(USB_FS_IRQn); // enable USB interrupt
  USB->BCDR |= USB_BCDR_DPPU;   // switch on PU
}
